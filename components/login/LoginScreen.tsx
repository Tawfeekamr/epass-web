...
import { TApiError, TCountry, TLoginFormInputs } from '@/@types/login';
...
const LoginScreen: React.FC<{ onCountryChange: (country: string) => void }> = ({ onCountryChange }) => {
    ...
    return (
        <div className="relative flex items-center justify-center min-h-screen bg-gray-900 text-white">
            <MeteorEffect />
            <Button onClick={() => { toggleTheme(); showNotification('Theme Changed', `The theme has been changed to ${theme === 'light' ? 'dark' : 'light'} mode.`); }} className="absolute top-4 right-4">
                {theme === 'light' ? (
                        ...
                        <CardContent className="grid gap-4">
                            <div className="grid gap-2">
                                ...
                                <Select onValueChange={handleCountryChange}>
                                    <SelectTrigger className="w-full px-3 py-2 text-black dark:text-white dark:bg-gray-800 rounded-md">
                                ...
                                {errors.country && <span className="text-red-500 text-[10px]">{errors.country.message}</span>}
                            </div>
                            <div className="grid gap-2">
                            ...
                            <div className="grid gap-2">
                                ...
                                <Input
                                    ...
                                    className="w-full px-3 py-2 text-black dark:text-white dark:bg-gray-800 rounded-md"
                                />
                                {errors.password && <span data-testid="api-error" className="text-red-500 text-[10px]">{errors.password.message}</span>}
                            </div>
                            {errors.apiError && (
                                <div data-testid="api-error" className="text-red-500 text-[10px]">{errors.apiError.message}</div>
                            )}
                        </CardContent>
                        <CardFooter>
